import { c as createLucideIcon } from "./index-BUnhKn-w.js";
/**
...
function getTodayString() {
  return dateToString(/* @__PURE__ */ new Date());
}
...
function hexToRgba(hex, opacity) {
  ...
  const r2 = Number.parseInt(clean.substring(0, 2), 16);
...
function timeToPixels(time, dayStartHour = 0) {
...
function getCurrentTimePixels(dayStartHour) {
...
function doBlocksOverlap(aStart, aDuration, bStart, bDuration) {
  const toMin = (t) => {
...
function getWeekDates(date, startOnMonday) {
...
function getMonthGrid(year, month, startOnMonday) {
  const firstDay = new Date(year, month, 1);
  const lastDay = new Date(year, month + 1, 0);
  const startDow = firstDay.getDay();
  const startOffset = startOnMonday ? startDow === 0 ? 6 : startDow - 1 : startDow;
  const days = [];
...
export {
  ...
  formatDuration as b,
  formatPrice as c,
  generateTimeSlots as d,
  getCurrentTimePixels as e,
  formatDate as f,
  getTodayString as g,
  durationToPixels as h,
  hexToRgba as i,
  getMonthGrid as j,
  dateToString as k,
  getWeekDates as l,
